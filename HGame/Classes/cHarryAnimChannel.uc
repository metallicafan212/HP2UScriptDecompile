...
var HProp propTemp;
// Omega: Unused, and this isn't even how you would want to do this anyway
//var int LastAnimFrame;

// Omega: Used to track if we casted during stateCast
// This restores the old but technically bugged behavior that if the cast 
// was interrupted, it would spawn the spell anyway. That means in stock if you
// take damage the instant you try to cast, the engine would believe that you passed
// frame 0.1 in Cast and send the notify out anyway. This also makes Pause Casting
// identical to stock again.
var bool bCastedInState;
...
state stateCast
{
	...
	function EndState()
	{
		// Omega: Track if we casted during the state or not, and cast if canceled early
		// This isn't just for the sake of making pause casts work like stock, but even
		// regular gameplay is affected
		if(!bCastedInState)
		{
			Cast();
		}
