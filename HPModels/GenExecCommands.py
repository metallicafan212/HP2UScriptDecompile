#AdamJD: Python script to generate the majority of the mesh, anim and texture exec commands
import os;
...
def GenExecCommands():
    ...
    for subdir, dirs, files in os.walk("Textures"):
         ...
         for fileTex in files:
            ...
            texString = ("//texture import (" + texName + ") -AdamJD \n"
                         "#exec Texture Import File=Textures\\" + fileTex + " Name=" + texName + " COMPRESSION=0 UPSCALE=1 Mips=1 Flags=" + localFlags + " Group=Skins \n"
    ...
    with open("ExecCommandImports.uc", "w+") as f:
        f.write("//this is just a temp file to have most of the exec commands in one place, the file does not get compiled -AdamJD \n\n");
        f.write("\n");
        f.write("//------------------------------------------------------------------------------------------------------------------------------------------------ \n");
