...
function Paint(Canvas C, float X, float Y)
{
	...
	if(bAllSelected)
	{
		DrawStretchedTexture(C, Offset + 1, fTextY, W, H, Texture'UWindow.WhiteTexture');
	...
	ClipText(C, Offset + 1, fTextY,  Value);
	...
	if(bShowCaret)
		// Metallicafan212:	There was a -1 here, and that caused the console caret to be off by one character at large resolutions
		ClipText(C, Offset + W, fTextY, "|");
}
