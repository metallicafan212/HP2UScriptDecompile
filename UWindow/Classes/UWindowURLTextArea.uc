...
function TextAreaClipText(Canvas C, float DrawX, float DrawY, coerce string S, optional bool bCheckHotkey)
{
	...
	local float fClickX, fClickY;
	local string sText, NextBlock;
	local byte bLink;
	...
	while(sText != "")
	{
		ProcessText(C, sText, NextBlock, W, H, bLink);
		if(bLink != 0)
		{
			C.DrawColor.R = 0;
			C.DrawColor.G = 0;
		...
		GetMouseXY(fClickX, fClickY);
		bOverLink = bLink != 0 && DrawX < fClickX && DrawX + W > fClickX && DrawY < fClickY && DrawY + H > fClickY;
