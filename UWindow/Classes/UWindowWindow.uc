...
{
	var Texture tex;
	var Texture tex2; //got added after the proto -AdamJD
	var int HotX;
	var int HotY;
	var int Tex2XOffset; //got added after the proto -AdamJD
	var int Tex2YOffset; //got added after the proto -AdamJD
...
final function PaintClients(Canvas C, float X, float Y)
{
	...
	while(Child != None)
	{
		...
		if(bUWindowActive || Child.bLeaveOnscreen)
		{
			...
			C.OrgY = C.OrgY + Child.WinTop 	* Root.HGUIScale;//* Root.GUIScale;

			if(!Child.bNoClip)
			{
				C.ClipX = FMin(WinWidth - Child.WinLeft, Child.WinWidth) * Root.GUIScale;
				C.ClipY = FMin(WinHeight - Child.WinTop, Child.WinHeight) * Root.HGUIScale;//Root.GUIScale;
									  ...
									  float tX, float tY, float tW, float tH, texture Tex ) 
{
	...
	C.SetClip(ClippingRegion.W * Root.GUIScale, ClippingRegion.H * Root.HGUIScale);//Root.GUIScale);
...
final function DrawClippedActor( Canvas C, float X, float Y, Actor A, bool WireFrame, rotator RotOffset, vector LocOffset )
{
	...
	C.DrawClippedActor(A, WireFrame, ClippingRegion.W * Root.GUIScale, ClippingRegion.H * Root.HGUIScale, C.OrgX + ClippingRegion.X * Root.GUIScale, C.OrgY + ClippingRegion.Y * Root.HGUIScale, True);
}
